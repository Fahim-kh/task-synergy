    ...
    <div class=" app-content">
        <div class="side-app">
            ...
            <div class="page-header">
                <h4 class="page-title"><i class="fe fe-grid mr-1"></i> Edit FAQ's</h4>
                <ol class="breadcrumb">
                    <li class="breadcrumb-item"><a href="#"> FAQ's</a></li>
                    <li class="breadcrumb-item active" aria-current="page">Edit FAQ's</li>
                </ol>
            </div>
            <!--page header-->
            <div class="row">
                        ...
                        <div class="card-body">
                            <form method="post" action="{{ route('faq.update', $faq->id) }}">
								...
								<div class="form-group row  @error('type') has-error @enderror">
                                    ...
                                    <div class="col-md-10">
                                        <select name="type" id="type" class="form-control">
                                            <option value="default" {{ ($faq->type == 'default')? 'selected' : '' }}>Default</option>
                                            <option value="Over-Night Care" {{ ($faq->type == 'Over-Night Care')? 'selected' : '' }}>Over-Night Care</option>
                                            <option value="Live-In Care" {{ ($faq->type == 'Live-In Care')? 'selected' : '' }}>Live-In Care</option>
                                            <option value="Live-In Care Placements" {{ ($faq->type == 'Live-In Care Placements')? 'selected' : '' }}>Live-In Care Placements</option>
                                            <option value="Funding & Process" {{ ($faq->type == 'Funding & Process')? 'selected' : '' }}>Funding & Process</option>
                                        </select>
                                    </div>
                                    @error('type')
                                ...
                                <div class="form-group  row  @error('answer') has-error @enderror">
                                    ...
                                    <div class="col-md-10">
                                        <textarea class="form-control answer" name="answer" id="word_count">{!! $faq->answer !!}</textarea>
                                    </div>
                                    @error('answer')
                                        <div class="label label-danger">{{ $message }}</div>
                                    @enderror
